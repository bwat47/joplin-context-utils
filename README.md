...
## Clean up pasted list items

When you paste a list after an existing list marker, this setting prevents duplicate bullets, numbers, or task boxes. For example, pasting `- foo` on a `- ` line produces `- foo` instead of `- - foo`.

It also adjusts the pasted list to the current list's indentation and style, and renumbers numbered items when needed. Nested items keep their structure.
...
- Clean up pasted list items (off by default; see [Clean up pasted list items](#clean-up-pasted-list-items)).

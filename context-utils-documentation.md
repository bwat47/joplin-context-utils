...
- `getPastedLineChanges(doc, pasteFrom, pastedText, linePrefix, tabSize, formatIndent)` - one pass over the later non-blank pasted lines, emitting at most one edit per line (leading whitespace, plus the marker token when it changes):
    ...
    - Lines are grouped by sibling item (`groupPastedLines`). A group's child and continuation lines move only if they would no longer be nested under the item's new marker (`getChildShift`): if the smallest child indentation falls outside the item's new content column to `MAX_CHILD_OFFSET` (3) columns past it, the whole subtree shifts the minimum amount into range. So 4-space or tab-indented children stay put when a marker changes between `- ` and `N. `, while 2-space children under a widened marker move out by one column. The pasted text is also parsed with `@lezer/markdown` (`getPastedItemBlocks`) to find each item's direct child blocks and child list items. The shift is limited so all of them stay in range, since a deeper child can fall out of range while the least indented one does not (e.g. a child item 3 columns past a `10. ` marker's content, below a paragraph, when narrowing to `- `). An item that directly contains an indented code block instead shifts its children by exactly the content column change, because a code block's meaning depends on its exact offset from the content column. Code blocks nested in a child item move with that child
